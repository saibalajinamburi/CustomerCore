...
> **18 phases completed | 307+ tests passing | 52k+ tickets | 7 languages | 6 AI agents | Live on HF Spaces**
...
| Phase 17-18 | Production fixes, Supabase cloud migration |
...
Q: Why didn't you perform PEFT/QLoRA fine-tuning on the LLM, and is it possible to do so now?
   A: We omitted QLoRA fine-tuning for two primary reasons:
   1. Tabular ML Superiority: Tabular ML algorithms (like Random Forest, XGBoost, and LightGBM) consistently outperform LLMs on structured tabular data in terms of accuracy, training speed, and runtime latency. Fine-tuning a heavy generative LLM for tabular churn prediction is a poor architectural choice.
   2. Hardware Constraints: QLoRA fine-tuning of an LLM requires substantial GPU resources (at least 16GB–24GB VRAM) for 7B/8B parameter models, which are unavailable on free-tier hosting environments.
   However, it is entirely possible to implement QLoRA now: we would spin up a cost-effective cloud GPU instance (e.g., RunPod or Google Colab), export historical tickets from Supabase, write a training loop using PyTorch/TRL to adapt a model like Gemma-2B-Instruct, quantize the adapters to 4-bit, and host them on our Hugging Face Space. I would target intent/category classification rather than churn prediction, since that is where a fine-tuned LLM actually has a clear reasoning advantage over classical ML.
